 ...
 * - FAILED: 실패
 */
export type TodoStatus = "ONGOING" | "COMPLETED" | "FAILED" | "PENDING";
...
/**
 * Firestore 문서를 Todo 객체로 변환하는 함수입니다.
 * @param {DocumentSnapshot<Todo>} doc - Firestore 문서
 * @return {Todo} Todo 객체
 */
export function docToTodo(doc: DocumentSnapshot): Todo {
  const data = doc.data();
  if (!data) {
